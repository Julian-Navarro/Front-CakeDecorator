...
export default function CourseListCard ({ course, handlerEditCourse, path, type, category, description, id, img, price, title, videos }) {
    const gradientColorBg = category === "Curso" ? "#DFFDFF" :"#DEFFD6"
    const newTitle = title.length > 80 ? `${title.slice(0, 80)}...` : title
    return (
        <DivGradient boxSh="0 0 .8rem .2rem #DCDCDC"bd="gray"wd="18rem" hg="27rem" flexDir="column" jfCont="space-between" pd="0 0 .7rem 0"mt="1rem" mr="1rem" bgGR1="transparent" bgGR2={gradientColorBg}>
